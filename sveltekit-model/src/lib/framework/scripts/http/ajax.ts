...
export class Ajax {
  ...
  private static createRequestOptions(requestOptions:AjaxRequestInit): AjaxRequestInit { 
    let reqOptions: AjaxRequestInit =  Object.assign(Ajax.createDefaultRequestOptions(), requestOptions); 
  ...
  private static async request(url: string, requestOptions: AjaxRequestInit): Promise<Response> {
    ...
    let reqOptions = Ajax.createRequestOptions(requestOptions);
    // AbortController 객체의 signal 속성을 fetch()의 signal 옵션에 전달합니다.  
    reqOptions.signal = controller.signal;
    const promise:Promise<Response> = fetch(url, reqOptions);
    if (reqOptions.signal) reqOptions.signal.addEventListener("abort", () => {
      console.log("aborted"); // 요청이 취소 되었음 
    });
    const timeout = setTimeout(() => controller.abort(), reqOptions.timeout); // 시간이 경과하면 요청을 취소한다. 
